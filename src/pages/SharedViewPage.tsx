...
import { Lock, Eye, StickyNote, Clock, ListTodo, KeyRound, ShieldAlert, CheckCircle, MessageCircle, FileText, Download } from 'lucide-react';
                        ...
                        {colNotes.map((note: any, i: number) => {
                          ...
                          return (
                            <div key={i} className="shared-item-wrapper relative p-3 rounded-xl border border-gray-200/50 dark:border-gray-700/30 bg-white/80 dark:bg-gray-800/50 shadow-sm"
                              ...
                              {(() => {
                                const list = Array.isArray(note.fichiers) ? [...note.fichiers] : [];
                                if (note.fichier && note.fichier.url && !list.find((f: any) => f.url === note.fichier.url)) {
