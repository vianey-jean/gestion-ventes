...
const CookieConsent: React.FC = () => {
  ...
  const Toggle = ({ enabled, onToggle, disabled = false }: { enabled: boolean; onToggle: () => void; disabled?: boolean }) => (
    <button
      ...
      disabled={disabled}
      className={`relative w-11 h-6 rounded-full transition-all duration-300 ${disabled
  ...
  return (
    <AnimatePresence>
      {visible && (
        /* ✅ MODIFICATION : position plus à gauche + responsive */
        <motion.div
          initial={{ y: 100, opacity: 0, scale: 0.95 }}
          animate={{ y: 0, opacity: 1, scale: 1 }}
          exit={{ y: 100, opacity: 0, scale: 0.95 }}
          transition={{ type: 'spring', damping: 22, stiffness: 220 }}
          className="fixed bottom-4 left-4 right-4 sm:left-6 sm:right-auto z-[9999] w-auto sm:max-w-2xl"
        >
