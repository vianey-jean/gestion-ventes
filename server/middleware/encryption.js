...
function readJsonDecrypted(filePath) {
  try {
    ...
    if (parsed && parsed.__encrypted === true) {
      if (config.key) {
        try {
      ...
      // Encrypted payload but no key available: never return the raw envelope
      console.error(`Encrypted file without available key: ${filePath}`);
      return null;
    }
